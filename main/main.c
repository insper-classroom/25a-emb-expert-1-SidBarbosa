...
#define WIFI_SSID "Sidney's Galaxy S22"
#define WIFI_PASSWORD "sidney123"
...
static float read_ldr(void) {
    ...
    float ldr_value = (voltage / 3.3f) * 100.0f;
    return ldr_value;
}
...
static void led(MQTT_CLIENT_DATA_T *state, bool on){
    const char* message = on ? "On" : "Off";
    if (on) gpio_put(LED, 1);
    else gpio_put(LED, 0);
    mqtt_publish(state->mqtt_client_inst, full_topic(state, "/led"), message, strlen(message), MQTT_PUBLISH_QOS, MQTT_PUBLISH_RETAIN, pub_request_cb, state);
    INFO_printf("LED %s\n", message);
}
...
static void sub_unsub_topics(MQTT_CLIENT_DATA_T* state, bool sub) {
    ...
    mqtt_sub_unsub(state->mqtt_client_inst, full_topic(state, "/belar/led"), MQTT_SUBSCRIBE_QOS, cb, state, sub);
    mqtt_sub_unsub(state->mqtt_client_inst, full_topic(state, "/teste/led"), MQTT_SUBSCRIBE_QOS, cb, state, sub);
...
static void mqtt_incoming_data_cb(void *arg, const u8_t *data, u16_t len, u8_t flags) {
    ...
    if (strcmp(basic_topic, "/belar/led") == 0)
    {
    ...
    }else if (strcmp(basic_topic, "/teste/led") == 0) {
        if (lwip_stricmp((const char *)state->data, "On") == 0 || strcmp((const char *)state->data, "1") == 0)
            led(state, true);
        else if (lwip_stricmp((const char *)state->data, "Off") == 0 || strcmp((const char *)state->data, "0") == 0)
            led(state, false);
...
int main(void) {
    ...
    // Opcional: garantir que o LED comece desligado
    gpio_put(LED, 0);
